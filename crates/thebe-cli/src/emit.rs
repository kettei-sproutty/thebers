...
pub fn document(body: &str, styles: &[&str], head_extra: &str) -> String {
...
fn emit_root_mod(
  ...
  for page in pages {
    ...
    if page.params.is_empty() {
      writeln!(code, "  let body = routes::{mod_path}::render();")
        .expect("write to String");
      writeln!(code, "  let head_extra = routes::{mod_path}::head();")
        .expect("write to String");
    } else {
      let args = page
        .params
      ...
      writeln!(code, "  let body = routes::{mod_path}::render({args});")
        .expect("write to String");
      writeln!(code, "  let head_extra = routes::{mod_path}::head({args});")
        .expect("write to String");
    }
    ...
    writeln!(
      code,
      "  axum::response::Html(shell::document(&body, &all_styles, &head_extra))"
    )
    .expect("write to String");
    code.push_str("}\n");
  }
  ...
  if let Some(err) = root_error {
    ...
    writeln!(code, "  let body = routes::{err_mod}::render();")
      .expect("write to String");
    writeln!(code, "  let head_extra = routes::{err_mod}::head();")
      .expect("write to String");
    writeln!(
      code,
    ...
    .expect("write to String");
    code.push_str(
      "  (axum::http::StatusCode::NOT_FOUND, axum::response::Html(shell::document(&body, styles, &head_extra)))\n",
    );
    code.push_str("}\n");
  }
