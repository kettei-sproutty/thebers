...
impl Emitter {
  ...
  fn emit_module(&mut self, c: &CompiledComponent, route_params: &[String]) -> Result<(), CompileError> {
  ...
  fn emit_style_constants(&mut self, c: &CompiledComponent) -> Result<(), CompileError> {
        ...
        if style.scoped {
          if let Some(id) = &style.scope_id {
            ...
            let rewritten = rewrite_scoped_css(&style.content, id)
              .map_err(|reason| CompileError::CssScopeError {
                reason,
                span: style.span,
              })?;
...
fn rewrite_scoped_css(css: &str, scope_id: &str) -> Result<String, String> {
  ...
  stylesheet
    ...
    .map(|r| r.code)
    .map_err(|e| format!("CSS serialisation error: {e}"))
}
